...
NSInteger const QUIET_ZONE_SIZE = 4;
...
- (ZXBitMatrix *)encode:(NSString *)contents format:(ZXBarcodeFormat)format width:(NSInteger)width height:(NSInteger)height hints:(ZXEncodeHints *)hints error:(NSError **)error {
  ...
  if (width < 0 || height < 0) {
    [NSException raise:NSInvalidArgumentException format:@"Requested dimensions are too small: %ldx%ld", (long)width, (long)height];
  }

  ZXErrorCorrectionLevel *errorCorrectionLevel = [ZXErrorCorrectionLevel errorCorrectionLevelL];
  NSInteger quietZone = QUIET_ZONE_SIZE;
  if (hints != nil) {
...
- (ZXBitMatrix *)renderResult:(ZXQRCode *)code width:(NSInteger)width height:(NSInteger)height quietZone:(NSInteger)quietZone {
  ZXByteMatrix *input = code.matrix;
  if (input == nil) {
    return nil;
  }
  NSInteger inputWidth = input.width;
  NSInteger inputHeight = input.height;
  NSInteger qrWidth = inputWidth + (quietZone << 1);
  NSInteger qrHeight = inputHeight + (quietZone << 1);
  NSInteger outputWidth = MAX(width, qrWidth);
  NSInteger outputHeight = MAX(height, qrHeight);

  NSInteger multiple = MIN(outputWidth / qrWidth, outputHeight / qrHeight);
  // Padding includes both the quiet zone and the extra white pixels to accommodate the requested
  // dimensions. For example, if input is 25x25 the QR will be 33x33 including the quiet zone.
  // If the requested size is 200x160, the multiple will be 4, for a QR of 132x132. These will
  // handle all the padding from 100x100 (the actual QR) up to 200x160.
  NSInteger leftPadding = (outputWidth - (inputWidth * multiple)) / 2;
  NSInteger topPadding = (outputHeight - (inputHeight * multiple)) / 2;

  ZXBitMatrix *output = [[ZXBitMatrix alloc] initWithWidth:outputWidth height:outputHeight];

  for (NSInteger inputY = 0, outputY = topPadding; inputY < inputHeight; inputY++, outputY += multiple) {
