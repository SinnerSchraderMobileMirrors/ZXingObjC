...
const NSInteger ALPHANUMERIC_TABLE[96] = {
...
+ (NSInteger)calculateMaskPenalty:(ZXByteMatrix *)matrix {
...
+ (ZXQRCode *)encode:(NSString *)content ecLevel:(ZXErrorCorrectionLevel *)ecLevel hints:(ZXEncodeHints *)hints error:(NSError **)error {
  ...
  NSInteger provisionalBitsNeeded = headerBits.size
  ...
  NSInteger bitsNeeded = headerBits.size
  ...
  NSInteger numLetters = [mode isEqual:[ZXMode byteMode]] ? [dataBits sizeInBytes] : [content length];
  ...
  NSInteger numDataBytes = version.totalCodewords - ecBlocks.totalECCodewords;
  ...
  NSInteger dimension = version.dimensionForVersion;
  ZXByteMatrix *matrix = [[ZXByteMatrix alloc] initWithWidth:dimension height:dimension];
  NSInteger maskPattern = [self chooseMaskPattern:finalBits ecLevel:[qrCode ecLevel] version:[qrCode version] matrix:matrix error:error];
  if (maskPattern == -1) {
    return nil;
  }
...
+ (NSInteger)alphanumericCode:(NSInteger)code {
  if (code < sizeof(ALPHANUMERIC_TABLE) / sizeof(NSInteger)) {
...
+ (NSInteger)chooseMaskPattern:(ZXBitArray *)bits ecLevel:(ZXErrorCorrectionLevel *)ecLevel version:(ZXQRCodeVersion *)version matrix:(ZXByteMatrix *)matrix error:(NSError **)error {
  NSInteger minPenalty = INT_MAX;
  NSInteger bestMaskPattern = -1;

  for (NSInteger maskPattern = 0; maskPattern < NUM_MASK_PATTERNS; maskPattern++) {
    if (![ZXMatrixUtil buildMatrix:bits ecLevel:ecLevel version:version maskPattern:maskPattern matrix:matrix error:error]) {
      return -1;
    }
    NSInteger penalty = [self calculateMaskPenalty:matrix];
...
+ (ZXQRCodeVersion *)chooseVersion:(NSInteger)numInputBits ecLevel:(ZXErrorCorrectionLevel *)ecLevel error:(NSError **)error {
  // In the following comments, we use numbers of Version 7-H.
  for (NSInteger versionNum = 1; versionNum <= 40; versionNum++) {
    ZXQRCodeVersion *version = [ZXQRCodeVersion versionForNumber:versionNum];
    // numBytes = 196
    NSInteger numBytes = version.totalCodewords;
    // getNumECBytes = 130
    ZXQRCodeECBlocks *ecBlocks = [version ecBlocksForLevel:ecLevel];
    NSInteger numEcBytes = ecBlocks.totalECCodewords;
    // getNumDataBytes = 196 - 130 = 66
    NSInteger numDataBytes = numBytes - numEcBytes;
    NSInteger totalInputBytes = (numInputBits + 7) / 8;
...
+ (NSInteger)totalInputBytes:(NSInteger)numInputBits version:(ZXQRCodeVersion *)version mode:(ZXMode *)mode {
  NSInteger modeInfoBits = 4;
  NSInteger charCountBits = [mode characterCountBits:version];
  NSInteger headerBits = modeInfoBits + charCountBits;
  NSInteger totalBits = numInputBits + headerBits;
...
+ (BOOL)terminateBits:(NSInteger)numDataBytes bits:(ZXBitArray *)bits error:(NSError **)error {
  NSInteger capacity = numDataBytes << 3;
  if ([bits size] > capacity) {
    NSDictionary *userInfo = @{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"data bits cannot fit in the QR Code %ld > %ld", (long)[bits size], (long) (long)capacity]};

    if (error) *error = [[NSError alloc] initWithDomain:ZXErrorDomain code:ZXWriterError userInfo:userInfo];
    return NO;
  }
  for (NSInteger i = 0; i < 4 && [bits size] < capacity; ++i) {
    [bits appendBit:NO];
  }
  NSInteger numBitsInLastByte = [bits size] & 0x07;
  if (numBitsInLastByte > 0) {
    for (NSInteger i = numBitsInLastByte; i < 8; i++) {
      [bits appendBit:NO];
    }
  }
  NSInteger numPaddingBytes = numDataBytes - [bits sizeInBytes];
...
+ (BOOL)numDataBytesAndNumECBytesForBlockID:(NSInteger)numTotalBytes numDataBytes:(NSInteger)numDataBytes numRSBlocks:(NSInteger)numRSBlocks blockID:(NSInteger)blockID numDataBytesInBlock:(NSInteger[])numDataBytesInBlock numECBytesInBlock:(NSInteger[])numECBytesInBlock error:(NSError **)error {
  ...
  NSInteger numRsBlocksInGroup2 = numTotalBytes % numRSBlocks;
  NSInteger numRsBlocksInGroup1 = numRSBlocks - numRsBlocksInGroup2;
  NSInteger numTotalBytesInGroup1 = numTotalBytes / numRSBlocks;
  NSInteger numTotalBytesInGroup2 = numTotalBytesInGroup1 + 1;
  NSInteger numDataBytesInGroup1 = numDataBytes / numRSBlocks;
  NSInteger numDataBytesInGroup2 = numDataBytesInGroup1 + 1;
  NSInteger numEcBytesInGroup1 = numTotalBytesInGroup1 - numDataBytesInGroup1;
  NSInteger numEcBytesInGroup2 = numTotalBytesInGroup2 - numDataBytesInGroup2;
...
+ (ZXBitArray *)interleaveWithECBytes:(ZXBitArray *)bits numTotalBytes:(NSInteger)numTotalBytes numDataBytes:(NSInteger)numDataBytes numRSBlocks:(NSInteger)numRSBlocks error:(NSError **)error {
  ...
  NSInteger dataBytesOffset = 0;
  NSInteger maxNumDataBytes = 0;
  NSInteger maxNumEcBytes = 0;

  // Since, we know the number of reedsolmon blocks, we can initialize the vector with the number.
  NSMutableArray *blocks = [NSMutableArray arrayWithCapacity:numRSBlocks];

  for (NSInteger i = 0; i < numRSBlocks; ++i) {
    NSInteger numDataBytesInBlock[1];
    NSInteger numEcBytesInBlock[1];
    ...
    NSInteger size = numDataBytesInBlock[0];
  ...
  for (NSInteger i = 0; i < maxNumDataBytes; ++i) {
    for (ZXBlockPair *block in blocks) {
      int8_t *dataBytes = block.dataBytes;
      NSInteger length = block.length;
      if (i < length) {
  ...
  for (NSInteger i = 0; i < maxNumEcBytes; ++i) {
    for (ZXBlockPair *block in blocks) {
      int8_t *ecBytes = block.errorCorrectionBytes;
      NSInteger length = block.errorCorrectionLength;
      if (i < length) {
  ...
  if (numTotalBytes != [result sizeInBytes]) {
    NSDictionary *userInfo = @{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Interleaving error: %ld and %ld differ.", (long)numTotalBytes, (long)[result sizeInBytes]]};
...
+ (int8_t *)generateECBytes:(int8_t[])dataBytes numDataBytes:(NSInteger)numDataBytes numEcBytesInBlock:(NSInteger)numEcBytesInBlock {
  ...
  for (NSInteger i = numDataBytes; i < toEncodeLen; i++) {
    toEncode[i] = 0;
  }
  ...
  for (NSInteger i = 0; i < numEcBytesInBlock; i++) {
...
+ (BOOL)appendLengthInfo:(NSInteger)numLetters version:(ZXQRCodeVersion *)version mode:(ZXMode *)mode bits:(ZXBitArray *)bits error:(NSError **)error {
  NSInteger numBits = [mode characterCountBits:version];
  if (numLetters >= (1 << numBits)) {
    NSDictionary *userInfo = @{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"%ld is bigger than %d", (long)numLetters, ((1 << numBits) - 1)]};
