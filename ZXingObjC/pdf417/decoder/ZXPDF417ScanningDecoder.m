...
+ (ZXDecoderResult *)createDecoderResultFromAmbiguousValues:(int)ecLevel codewords:(NSMutableArray *)codewords
  ...
  NSMutableArray *ambiguousIndexCount = [NSMutableArray array];
  for (int i = 0; i < [ambiguousIndexes count]; i++) {
    [ambiguousIndexCount addObject:@0];
  }

  int tries = 100;
  while (tries-- > 0) {
    for (int i = 0; i < [ambiguousIndexCount count]; i++) {
      codewords[[ambiguousIndexes[i] intValue]] = ambiguousIndexValues[i][[ambiguousIndexCount[i] intValue] ];
    }
    ...
    if ([ambiguousIndexCount count] == 0) {
      return nil;
    }
    for (int i = 0; i < [ambiguousIndexCount count]; i++) {
      if ([ambiguousIndexCount[i] intValue] < [ambiguousIndexValues[i] count] - 1) {
        ambiguousIndexCount[i] = @([ambiguousIndexCount[i] intValue] + 1);
        break;
      } else {
        ambiguousIndexCount[i] = @0;
        if (i == [ambiguousIndexCount count] - 1) {
          return nil;
        }
      }
    }
