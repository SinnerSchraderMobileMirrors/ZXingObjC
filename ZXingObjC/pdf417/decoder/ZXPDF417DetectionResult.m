...
@property (nonatomic, strong) NSMutableArray *detectionResultColumns;
...
- (id)initWithBarcodeMetadata:(ZXPDF417BarcodeMetadata *)barcodeMetadata boundingBox:(ZXPDF417BoundingBox *)boundingBox {
  ...
  if (self) {
    ...
    _detectionResultColumns = [NSMutableArray arrayWithCapacity:_barcodeColumnCount + 2];
    for (int i = 0; i < _barcodeColumnCount + 2; i++) {
      [_detectionResultColumns addObject:[NSNull null]];
    }
  }
...
- (int)imageStartRow:(int)barcodeColumn {
  while (barcodeColumn > 0) {
    ZXPDF417DetectionResultColumn *detectionResultColumn = self.detectionResultColumns[--barcodeColumn];
...
- (ZXPDF417DetectionResultColumn *)detectionResultColumn:(int)barcodeColumn {
  return self.detectionResultColumns[barcodeColumn];
}
...
- (NSArray *)detectionResultColumns {
  [self adjustIndicatorColumnRowNumbers:self.detectionResultColumns[0]];
  [self adjustIndicatorColumnRowNumbers:self.detectionResultColumns[self.barcodeColumnCount + 1]];
  ...
  return self.detectionResultColumns;
}
...
- (int)adjustRowNumbers {
  ...
  for (int barcodeColumn = 1; barcodeColumn < self.barcodeColumnCount + 1; barcodeColumn++) {
    NSArray *codewords = [self.detectionResultColumns[barcodeColumn] codewords];
...
- (int)adjustRowNumbersFromRRI {
  if (!self.detectionResultColumns[self.barcodeColumnCount + 1]) {
    return 0;
  }
  int unadjustedCount = 0;
  NSArray *codewords = [self.detectionResultColumns[self.barcodeColumnCount + 1] codewords];
  for (int codewordsRow = 0; codewordsRow < [codewords count]; codewordsRow++) {
    if (!codewords[codewordsRow]) {
      continue;
    }
    ...
    for (int barcodeColumn = self.barcodeColumnCount + 1;
         ...
         barcodeColumn--) {
      ZXPDF417Codeword *codeword = [self.detectionResultColumns[barcodeColumn] codewords][codewordsRow];
      if (codeword) {
...
- (int)adjustRowNumbersFromLRI {
  if (!self.detectionResultColumns[0]) {
    return 0;
  }
  int unadjustedCount = 0;
  NSArray *codewords = [self.detectionResultColumns[0] codewords];
  for (int codewordsRow = 0; codewordsRow < [codewords count]; codewordsRow++) {
    if (!codewords[codewordsRow]) {
      continue;
    }
...
- (void)adjustRowNumbers:(int)barcodeColumn codewordsRow:(int)codewordsRow codewords:(NSArray *)codewords {
  ...
  NSArray *previousColumnCodewords = [self.detectionResultColumns[barcodeColumn - 1] codewords];
  NSArray *nextColumnCodewords = previousColumnCodewords;
  if (self.detectionResultColumns[barcodeColumn + 1]) {
    nextColumnCodewords = [self.detectionResultColumns[barcodeColumn + 1] codewords];
  }
