...
const NSInteger SYMBOL_WIDEST[5] = {7, 5, 4, 3, 1};
const NSInteger EVEN_TOTAL_SUBSET[5] = {4, 20, 52, 104, 204};
const NSInteger GSUM[5] = {0, 348, 1388, 2948, 3988};

const NSInteger WEIGHTS[23][8] = {
...
const NSInteger FINDER_PAT_A = 0;
const NSInteger FINDER_PAT_B = 1;
const NSInteger FINDER_PAT_C = 2;
const NSInteger FINDER_PAT_D = 3;
const NSInteger FINDER_PAT_E = 4;
const NSInteger FINDER_PAT_F = 5;

#define FINDER_PATTERN_SEQUENCES_LEN 10
#define FINDER_PATTERN_SEQUENCES_SUBLEN 11
const NSInteger FINDER_PATTERN_SEQUENCES[FINDER_PATTERN_SEQUENCES_LEN][FINDER_PATTERN_SEQUENCES_SUBLEN] = {
...
const NSInteger MAX_PAIRS = 11;

@interface ZXRSSExpandedReader () {
  NSInteger startEnd[2];
//  NSInteger currentSequence[LONGEST_SEQUENCE_SIZE];
  BOOL startFromEven;
}
...
- (NSMutableArray *)decodeRow2pairs:(NSInteger)rowNumber row:(ZXBitArray *)row {
  while (YES) {
...
- (NSMutableArray *)checkRows:(NSMutableArray *)collectedRows current:(NSInteger)currentRow {
  for (NSInteger i = currentRow; i < [self.rows count]; i++) {
    ZXExpandedRow *row = self.rows[i];
    [self.pairs removeAllObjects];
    NSInteger size = [collectedRows count];
...
- (BOOL)isValidSequence:(NSArray *)pairs {
  for (NSInteger i = 0, sz = 2; i < FINDER_PATTERN_SEQUENCES_LEN; i++, sz++) {
    if ([self.pairs count] > sz) {
      continue;
    }

    BOOL stop = YES;
    for (NSInteger j = 0; j < [self.pairs count]; j++) {
...
- (void)storeRow:(NSInteger)rowNumber wasReversed:(BOOL)wasReversed {
...
- (BOOL)checkChecksum {
  ...
  NSInteger checksum = [firstCharacter checksumPortion];
  NSInteger s = 2;

  for (NSInteger i = 1; i < self.pairs.count; ++i) {
  ...
  NSInteger checkCharacterValue = 211 * (s - 4) + checksum;

  return checkCharacterValue == checkCharacter.value;
}

- (NSInteger)nextSecondBar:(ZXBitArray *)row initialPos:(NSInteger)initialPos {
...
- (ZXExpandedPair *)retrieveNextPair:(ZXBitArray *)row previousPairs:(NSMutableArray *)previousPairs rowNumber:(NSInteger)rowNumber {
...
- (BOOL)findNextPair:(ZXBitArray *)row previousPairs:(NSMutableArray *)previousPairs forcedOffset:(NSInteger)forcedOffset {
  const NSInteger countersLen = self.decodeFinderCountersLen;
...
- (void)reverseCounters:(NSInteger *)counters length:(NSUInteger)length {
  for(NSInteger i = 0; i < length / 2; ++i){
    NSInteger tmp = counters[i];
    counters[i] = counters[length - i - 1];
    counters[length - i - 1] = tmp;
  }
}

- (ZXRSSFinderPattern *)parseFoundFinderPattern:(ZXBitArray *)row rowNumber:(NSInteger)rowNumber oddPattern:(BOOL)oddPattern {
  // Actually we found elements 2-5.
  NSInteger firstCounter;
  NSInteger start;
  NSInteger end;
  ...
  if (oddPattern) {
    ...
    NSInteger firstElementStart = startEnd[0] - 1;
    // Locate element 1
  ...
  for (NSInteger i = 1; i < countersLen; i++) {
    counters[i] = self.decodeFinderCounters[i - 1];
  }

  counters[0] = firstCounter;
  NSInteger value = [ZXAbstractRSSReader parseFinderValue:counters countersSize:countersLen
...
- (ZXDataCharacter *)decodeDataCharacter:(ZXBitArray *)row pattern:(ZXRSSFinderPattern *)pattern isOddPattern:(BOOL)isOddPattern leftChar:(BOOL)leftChar {
  ...
  NSInteger numModules = 17; //left and right data characters have all the same length
  ...
  NSInteger weightRowNumber = 4 * pattern.value + (isOddPattern ? 0 : 2) + (leftChar ? 0 : 1) - 1;
  ...
  for (NSInteger i = self.oddCountsLen - 1; i >= 0; i--) {
    if ([self isNotA1left:pattern isOddPattern:isOddPattern leftChar:leftChar]) {
      NSInteger weight = WEIGHTS[weightRowNumber][2 * i];
  ...
  for (NSInteger i = self.evenCountsLen - 1; i >= 0; i--) {
    if ([self isNotA1left:pattern isOddPattern:isOddPattern leftChar:leftChar]) {
      NSInteger weight = WEIGHTS[weightRowNumber][2 * i + 1];
      evenChecksumPortion += self.evenCounts[i] * weight;
    }
    //evenSum += self.evenCounts[i];
  }
  NSInteger checksumPortion = oddChecksumPortion + evenChecksumPortion;

  if ((oddSum & 0x01) != 0 || oddSum > 13 || oddSum < 4) {
    return nil;
  }

  NSInteger group = (13 - oddSum) / 2;
  NSInteger oddWidest = SYMBOL_WIDEST[group];
  ...
  NSInteger tEven = EVEN_TOTAL_SUBSET[group];
  NSInteger gSum = GSUM[group];
  NSInteger value = vOdd * tEven + vEven + gSum;
...
- (BOOL)adjustOddEvenCounts:(NSInteger)numModules {
