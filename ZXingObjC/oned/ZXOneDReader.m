...
NSInteger const INTEGER_MATH_SHIFT = 8;
NSInteger const PATTERN_MATCH_RESULT_SCALE_FACTOR = 1 << INTEGER_MATH_SHIFT;
...
- (ZXResult *)decode:(ZXBinaryBitmap *)image hints:(ZXDecodeHints *)hints error:(NSError **)error {
  ...
  if (result) {
    ...
    if (tryHarder && [image rotateSupported]) {
      ...
      if (points != nil) {
        NSInteger height = [rotatedImage height];
...
- (ZXResult *)doDecode:(ZXBinaryBitmap *)image hints:(ZXDecodeHints *)hints error:(NSError **)error {
  ...
  NSInteger middle = height >> 1;
  BOOL tryHarder = hints != nil && hints.tryHarder;
  NSInteger rowStep = MAX(1, height >> (tryHarder ? 8 : 5));
  NSInteger maxLines;
  if (tryHarder) {
    maxLines = height;
  } else {
  ...
  for (NSInteger x = 0; x < maxLines; x++) {
    NSInteger rowStepsAboveOrBelow = (x + 1) >> 1;
    BOOL isAbove = (x & 0x01) == 0;
    NSInteger rowNumber = middle + rowStep * (isAbove ? rowStepsAboveOrBelow : -rowStepsAboveOrBelow);
...
+ (BOOL)recordPattern:(ZXBitArray *)row start:(NSInteger)start counters:(NSInteger[])counters countersSize:(NSInteger)countersSize {
  ...
  memset(counters, 0, numCounters * sizeof(NSInteger));
...
+ (BOOL)recordPatternInReverse:(ZXBitArray *)row start:(NSInteger)start counters:(NSInteger[])counters countersSize:(NSInteger)countersSize {
  NSInteger numTransitionsLeft = countersSize;
...
+ (NSInteger)patternMatchVariance:(NSInteger[])counters countersSize:(NSInteger)countersSize pattern:(NSInteger[])pattern maxIndividualVariance:(NSInteger)maxIndividualVariance {
  ...
  NSInteger unitBarWidth = (total << INTEGER_MATH_SHIFT) / patternLength;
  maxIndividualVariance = (maxIndividualVariance * unitBarWidth) >> INTEGER_MATH_SHIFT;
  NSInteger totalVariance = 0;

  for (NSInteger x = 0; x < numCounters; x++) {
    NSInteger counter = counters[x] << INTEGER_MATH_SHIFT;
