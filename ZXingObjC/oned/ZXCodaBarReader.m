...
static NSInteger MAX_ACCEPTABLE;
static NSInteger PADDING;

const NSInteger CODA_ALPHABET_LEN = 22;
const char CODA_ALPHABET[CODA_ALPHABET_LEN] = "0123456789-$:/.+ABCDTN";

/**
 * These represent the encodings of characters, as patterns of wide and narrow bars. The 7 least-significant bits of
 * each NSInteger correspond to the pattern of wide and narrow, with 1s representing "wide" and 0s representing narrow.
 */
const NSInteger CODA_CHARACTER_ENCODINGS_LEN = 20;
const NSInteger CODA_CHARACTER_ENCODINGS[CODA_CHARACTER_ENCODINGS_LEN] = {
...
const NSInteger MIN_CHARACTER_LENGTH = 3;

// official start and end patterns
const NSInteger STARTEND_ENCODING_LEN = 4;
...
@property (nonatomic, assign) NSInteger *counters;
@property (nonatomic, assign) NSInteger countersLen;
@property (nonatomic, assign) NSInteger counterLength;
...
+ (void)initialize {
  MAX_ACCEPTABLE = (NSInteger) (PATTERN_MATCH_RESULT_SCALE_FACTOR * 2.0f);
  PADDING = (NSInteger) (PATTERN_MATCH_RESULT_SCALE_FACTOR * 1.5f);
}
...
- (id)init {
  if (self = [super init]) {
    ...
    _countersLen = 80;
    _counters = (NSInteger *)malloc(_countersLen * sizeof(NSInteger));
    memset(_counters, 0, _countersLen * sizeof(NSInteger));
    _counterLength = 0;
  }
...
- (ZXResult *)decodeRow:(NSInteger)rowNumber row:(ZXBitArray *)row hints:(ZXDecodeHints *)hints error:(NSError **)error {
  ...
  NSInteger startOffset = [self findStartPattern];
  if (startOffset == -1) {
    if (error) *error = NotFoundErrorInstance();
    return nil;
  }

  NSInteger nextStart = startOffset;

  self.decodeRowResult = [NSMutableString string];
  do {
    NSInteger charOffset = [self toNarrowWidePattern:nextStart];
    if (charOffset == -1) {
      if (error) *error = NotFoundErrorInstance();
      return nil;
    }
  ...
  NSInteger trailingWhitespace = self.counters[nextStart - 1];
  NSInteger lastPatternSize = 0;
  for (NSInteger i = -8; i < -1; i++) {
  ...
  for (NSInteger i = 0; i < self.decodeRowResult.length; i++) {
  ...
  for (NSInteger i = startOffset; i < nextStart - 1; i++) {
    runningCount += self.counters[i];
  }
...
- (BOOL)validatePattern:(NSInteger)start {
  // First, sum up the total size of our four categories of stripe sizes;
  NSInteger sizes[4] = {0, 0, 0, 0};
  NSInteger counts[4] = {0, 0, 0, 0};
  NSInteger end = self.decodeRowResult.length - 1;

  // We break out of this loop in the middle, in order to handle
  // inter-character spaces properly.
  NSInteger pos = start;
  for (NSInteger i = 0; true; i++) {
    NSInteger pattern = CODA_CHARACTER_ENCODINGS[[self.decodeRowResult characterAtIndex:i]];
    for (NSInteger j = 6; j >= 0; j--) {
      // Even j = bars, while odd j = spaces. Categories 2 and 3 are for
      // long stripes, while 0 and 1 are for short stripes.
      NSInteger category = (j & 1) + (pattern & 1) * 2;
  ...
  for (NSInteger i = 0; true; i++) {
    ...
    for (NSInteger j = 6; j >= 0; j--) {
      ...
      // long stripes, while 0 and 1 are for short stripes.
      NSInteger category = (j & 1) + (pattern & 1) * 2;
      NSInteger size = self.counters[pos + j] << INTEGER_MATH_SHIFT;
...
- (void)counterAppend:(NSInteger)e {
  self.counters[self.counterLength] = e;
  self.counterLength++;
  if (self.counterLength >= self.countersLen) {
    NSInteger *temp = (NSInteger *)malloc(2 * self.counterLength * sizeof(NSInteger));
    memcpy(temp, self.counters, self.countersLen * sizeof(NSInteger));
    self.counters = temp;
    memset(self.counters, 0, 2 * self.counterLength * sizeof(NSInteger));
    self.countersLen = 2 * self.counterLength;
  }
}

- (NSInteger)findStartPattern {
  for (NSInteger i = 1; i < self.counterLength; i += 2) {
    NSInteger charOffset = [self toNarrowWidePattern:i];
    if (charOffset != -1 && [[self class] arrayContains:(char *)STARTEND_ENCODING length:STARTEND_ENCODING_LEN key:CODA_ALPHABET[charOffset]]) {
      ...
      NSInteger patternSize = 0;
      for (NSInteger j = i; j < i + 7; j++) {
...
+ (BOOL)arrayContains:(char *)array length:(NSUInteger)length key:(unichar)key {
  if (array != nil) {
...
- (NSInteger)toNarrowWidePattern:(NSInteger)position {
  NSInteger end = position + 7;
  if (end >= self.counterLength) {
    return -1;
  }

  NSInteger maxBar = 0;
  NSInteger minBar = INT_MAX;
  for (NSInteger j = position; j < end; j += 2) {
    NSInteger currentCounter = self.counters[j];
  ...
  NSInteger thresholdBar = (minBar + maxBar) / 2;

  NSInteger maxSpace = 0;
  NSInteger minSpace = INT_MAX;
  for (NSInteger j = position + 1; j < end; j += 2) {
    NSInteger currentCounter = self.counters[j];
  ...
  NSInteger thresholdSpace = (minSpace + maxSpace) / 2;

  NSInteger bitmask = 1 << 7;
  NSInteger pattern = 0;
  for (NSInteger i = 0; i < 7; i++) {
    NSInteger threshold = (i & 1) == 0 ? thresholdBar : thresholdSpace;
    bitmask >>= 1;
  ...
  for (NSInteger i = 0; i < CODA_CHARACTER_ENCODINGS_LEN; i++) {
